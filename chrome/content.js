...
async function getArticleInfoAsync(id, pageType) {
  ...
  const firstAuthorFirst2letters = nameParts[0].slice(0, 2).toLowerCase();
  const firstAuthorLast = nameParts[nameParts.length - 1];
  const firstAuthorCustom = `${firstAuthorLast}-${firstAuthorFirst2letters}`;
  const authors = [...entry.getElementsByTagName("name")].map((el) => el.textContent).join(", ");
  const publishedYear = entry.getElementsByTagName("published")[0].textContent.split('-')[0].slice(-2);
...
async function addCustomLinksAsync(id, articleInfo) {
  ...
  const fileName = result.filename_format
    ...
    .replace('${firstAuthorLast}', articleInfo.firstAuthorLast)
    .replace('${firstAuthorCustom}', articleInfo.firstAuthorCustom)
