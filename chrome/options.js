...
async function restoreOptionsAsync() {
  const result = await chrome.storage.sync.get({
    ...
    'filename_keys': '${title}, ${firstAuthor}, ${firstAuthorLast}, ${firstAuthorCustom}, ${publishedYear}, v${version}'
  });
  const filename_format = result.filename_format;
  const filename_keys = result.filename_keys;
  document.querySelector("#filename-format").innerText = filename_format;
  document.querySelector("#filename-keys").innerText = filename_keys;
